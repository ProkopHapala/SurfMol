...
// ==================================================================
// MULTI-REPLICA KERNELS (independent replicas, no ghosts)
//
// 2D NDRange: global = (GROUP_SIZE, nSys), local = (GROUP_SIZE, 1)
//   lid = get_local_id(0)  — atom index within replica (0..GROUP_SIZE-1)
//   iS  = get_global_id(1) — replica index (0..nSys-1)
//
// Shared buffers (same molecule = same topology, uploaded once):
//   radius[natoms], neighs[natoms], excl1[natoms], excl2[natoms],
//   fixmask[natoms], port_local[nnode*4], kflat[nnode*4], bk_slots[natoms]
//   — all indexed by local index (lid)
//
// Per-replica buffers (differ across replicas):
//   pos[nSys*natoms], quat[nSys*natoms], dpos_coll[nSys*natoms],
//   dpos_node[nSys*nnode], drot_node[nSys*nnode], dpos_neigh[nSys*nnode*4],
//   dpos_mom[nSys*natoms], dquat_mom[nSys*natoms]
//   — all indexed by i0a + lid or i0n + lid where i0a = iS*natoms, i0n = iS*nnode
//
// No ghost atoms: all neighbors are within the same workgroup.
// No bboxes/topology build needed: neighs/excl are used directly (identity mapping).
// ==================================================================

/// Multi-replica. Jacobi positional corrections for pairwise sphere collisions.
/// No ghosts — all atoms are in the workgroup. Shared excl/radius, per-replica pos/dpos_coll.
__kernel void compute_collision_multi(
    __global const float4* pos,        // [nSys * natoms]
    __global const float*  radius,     // [natoms] shared
    __global const int4*   excl1,      // [natoms] shared
    __global const int4*   excl2,      // [natoms] shared
    __global float4*       dpos_coll,  // [nSys * natoms]
    const int natoms,
    const float k_coll
) {
    int lid = get_local_id(0);
    ...
    __local float  l_rad[GROUP_SIZE];

    float4 pi4 = (lid < natoms) ? pos[ia] : (float4)(0.0f);
    float invMi = pi4.w;
    float ri = (lid < natoms) ? radius[lid] : 0.0f;  // shared radius

    if (invMi <= 1e-12f || ri <= 0.0f) { pi4 = (float4)(0.0f); invMi = 0.0f; ri = 0.0f; }
    l_pos[lid] = pi4;
    l_rad[lid] = ri;
    barrier(CLK_LOCAL_MEM_FENCE);

    if (lid >= natoms) { dpos_coll[ia] = (float4)(0.0f); return; }
    if (invMi <= 1e-12f || ri <= 0.0f) { dpos_coll[ia] = (float4)(0.0f); return; }

    float3 p = pi4.xyz;
    int4 e1 = excl1[lid];  // shared, local index
    int4 e2 = excl2[lid];  // shared, local index

    float3 sum = (float3)(0.0f);
    for (int j = 0; j < natoms; j++) {
        if (j == lid) continue;
        if (excluded8(j, e1, e2)) continue;
        ...
        if (d2 < r2 && d2 > 1e-16f) {
            float dist = sqrt(d2);
            float3 n = d / dist;
            float w_tot = invMi + invMj + 1e-12f;
            float dl = (rsum - dist) / w_tot;
            dl *= 0.5f;
            sum += n * (dl * invMi);
        }
    }
    dpos_coll[ia] = (float4)(sum, 0.0f);
}

/// Multi-replica. Massfull XPBD port forces (Current variant).
/// No ghosts — all neighbors in workgroup. Shared neighs/port_local/kflat, per-replica pos/quat/dpos.
__kernel void compute_ports_current_multi(
    __global const float4* pos,           // [nSys * natoms]
    __global const float4* quat,          // [nSys * natoms]
    __global const float*  radius,        // [natoms] shared
    __global const int4*   neighs,        // [natoms] shared (local indices 0..natoms-1)
    __global const float4* port_local,    // [nnode * 4] shared
    __global const float*  stiffness_flat,// [nnode * 4] shared
    __global float4*       dpos_node,     // [nSys * nnode]
    __global float4*       drot_node,     // [nSys * nnode]
    __global float4*       dpos_neigh,    // [nSys * nnode * 4]
    const int natoms,
    const int nnode_per_group,
    const float dt,
    const float rot_mass_scale
    ...
    int iS  = get_global_id(1);
    int i0a = iS * natoms;
    int i0n = iS * nnode_per_group;
    int i0n4 = iS * nnode_per_group * 4;
    int ia  = i0a + lid;
    ...
    int4 ng = neighs[lid];  // shared, local indices
    int* neighbors = (int*)&ng;
    int i4 = lid * 4;  // local node index * 4
    ...
    for (int k = 0; k < 4; k++) {
        ...
        int j_isnode = (jloc < nnode_per_group);
        float w_total = w_i + w_j + w_ang + alpha + 1e-12f;
        if (j_isnode) { w_total += w_ang; }
    ...
    drot_node[i0n + lid] = (float4)(sum_dtheta, 0.0f); // per-replica
}

/// Multi-replica. Apply position + quaternion corrections.
/// Shared bk_slots/port_local/tips, per-replica pos/quat/dpos.
__kernel void apply_corrections_multi(
    const int natoms,
    const int nnode_per_group,
    __global float4* pos,              // [nSys * natoms]
    __global float4* quat,             // [nSys * natoms]
    __global const int* fixmask,       // [natoms] shared
    __global const int4* bkSlots,      // [natoms] shared (local slot indices)
    __global const float4* dpos_node,  // [nSys * nnode]
    __global const float4* drot_node,  // [nSys * nnode]
    __global const float4* dpos_neigh, // [nSys * nnode * 4]
    __global const float4* dpos_coll,  // [nSys * natoms]
    __global float4* dpos_mom,         // [nSys * natoms]
    __global float4* dquat_mom,        // [nSys * natoms]
    const float relaxation,
    const float beta,
    const int massless_rot,
    __global const float4* port_local, // [nnode * 4] shared
    __global float4* tips              // [nSys * nnode * 4] per-replica (for massless)
) {
    int lid = get_local_id(0);
    int iS  = get_global_id(1);
    int i0a = iS * natoms;
    ...
    float invMi = pos[ia].w;
    if (invMi <= 1e-12f) return;

    int isnode = (lid < nnode_per_group);

    float3 dx_coll = dpos_coll[ia].xyz;  // per-replica
    float3 dx_port = (float3)(0.0f);
    if (isnode) {
        dx_port += dpos_node[i0n + lid].xyz;  // per-replica
    }

    int4 bk = bkSlots[lid];  // shared, local slot indices
    int* pbk = (int*)&bk;
    for (int k = 0; k < 4; k++) {
        int slot = pbk[k];
        if (slot >= 0) {
            dx_port += dpos_neigh[i0n4 + slot].xyz;  // per-replica offset
        }
    }

    float3 dx = dx_coll + dx_port;

    int msk = (fixmask != 0) ? fixmask[lid] : 0;  // shared
    if (msk & 1) dx.x = 0.0f;
    if (msk & 2) dx.y = 0.0f;
    if (msk & 4) dx.z = 0.0f;
    ...
    if (msk & 8) {
        ...
        if (fabs(p_old.z) > 1e-6f) { move.z -= p_old.z; }
        else { move.z = 0.0f; }
    }

    pos[ia].xyz += move;
    dpos_mom[ia] = (float4)(move, 0.0f);

    if (isnode) {
        if (msk & (1|2|4)) return;

        float3 dtheta = drot_node[i0n + lid].xyz * relaxation;  // per-replica
        float angle = length(dtheta);

        float4 q_old = quat[ia];  // per-replica
        float4 q_jacobi = q_old;
        ...
        quat[ia] = q_new;
        if (!massless_rot) dquat_mom[ia] = q_new - q_old;

        if (massless_rot && tips != (__global float4*)0 && port_local != (__global const float4*)0) {
            int i4 = lid * 4;  // local node index * 4
            float3 xi = pos[ia].xyz;
            for (int k = 0; k < 4; k++) {
                int idx = i4 + k;
                float3 r_local = port_local[idx].xyz;  // shared
                float3 r_arm = quat_rotate(q_new, r_local);
                tips[i0n4 + idx] = (float4)(xi + r_arm, 0.0f);  // per-replica
            }
        }
    }
}

/// Multi-replica. Zero correction buffers (dpos_coll, dpos_node, drot_node, dpos_neigh).
/// One thread per atom per replica.
__kernel void zero_corrections_multi(
    __global float4* dpos_coll,  // [nSys * natoms]
    __global float4* dpos_node,  // [nSys * nnode]
    __global float4* drot_node,  // [nSys * nnode]
    __global float4* dpos_neigh, // [nSys * nnode * 4]
