...
|---|---|---|---|---|
| **RAFF core engine** | `crates/libs/molff/src/raff.rs` | 1085 | **active** | `RaffTopology` (port geometry, neighbors, bond params, mass, inertia), `RaffState` (pos, vel, quat, omega), `RaffConfig` (dt, damping, orient mode), `NbConfig` (non-bonded). All 4 dynamics variants implemented: `step_force_md`, `step_xpbd`, `step_proximal`, `relax_force_md`, `relax_xpbd`. |
...
| **Non-bonded (LJ+Coulomb)** | `raff.rs:585` | — | **active** | `eval_nonbonded`: O(N²) pairwise LJ + Coulomb with 1-2/1-3 exclusion. Force clamping (`f_max`). No Morse, no compact-exp, no split-collision yet. |
...
| **Editor integration** | `crates/apps/editor/src/main.rs` | 1433 | **active** | `BondedFFMode::Raff` in `MolWorld`. `do_raff_step()` runs per-frame relaxation with spring drag, 2D constraint, port sync. GUI panel for RAFF settings. CLI flags `--raff`, `--2d`, `--atom-scale`. |
