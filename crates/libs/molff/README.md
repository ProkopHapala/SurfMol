...
type: rust-crate
title: molff
description: Intra-molecular forcefields — UFF (bonds/angles/dihedrals/inversions), rigid sp3 quaternion body dynamics, non-bonded LJ+Coulomb+H-bond with exclusion lists and PBC.
tags: [rust, crate, forcefield, uff, molecular-dynamics, rigid-body, nonbonded]
timestamp: 2026-08-25
---

# molff

Intra-molecular forcefield library: three independent engines that share the same data-oriented design (64-byte aligned arrays, parameter packing, cached intermediates) but have no interdependency. `surfmol` orchestrates them together; each can be used standalone with raw arrays.
...
- `surfmol` — `MolWorld` orchestrator that combines Uff + NonBondedFF + RigidSp3FF + SurfaceFolded
