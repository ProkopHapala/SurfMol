...
- **Debug artifacts location:** all generated plots (`.png`/`.svg`), trajectories (`.xyz`), and data tables (`.csv`/`.tsv`) go into `debug/<test_or_module_name>/` — a subfolder of `debug/` named after the test or module that produces them. Example: `debug/raff_ocl_smoke/convergence.png`, `debug/relax_xylitol_uff/fire.png`. **Never** save debug artifacts to `/tmp/` — they must be persistent and discoverable in the repo. The `<ref_file>` link in chat must use the `/home/prokohapala/git/SurfMol/debug/...` absolute path.
...
- **Build footprint** — workspace `Cargo.toml` profiles use `debug = 1` + `strip = "debuginfo"` (dev & release): drops `.debug_line`/`.debug_info`/`.debug_str` from binaries (~16× smaller debug binaries), `.eh_frame` survives so backtraces keep function names + panic `file:line`. Release adds `lto = "thin"`, `codegen-units = 1`, `incremental = true`, `debug-assertions = true`, `overflow-checks = true`. Ported from `blood_of_civilization/doc/AGENTS/notes/Memory_Issues/reduce_target_footprint_plan.md` A1. **Shared target dir** is machine-local (set via `CARGO_TARGET_DIR` env var or `~/.cargo/config.toml`); the repo `.cargo/config.toml` falls back to local `target/`. **Deps hygiene:** `image` is dev-only (tests use `image::RgbaImage` for PNG save); never put it in `[dependencies]` of a crate whose `src/` doesn't call `image::`.
