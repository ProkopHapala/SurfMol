...
|| `gridff_spammm.cl` | SPAMMM GridFF: B-spline, Poisson solver, `make_GridFF` texture build, `sampleGridFF_Bspline_points`, charge projection. **Requires** `common.cl`+`Forces.cl` first. | SPAMMM `kernels/gridFF.cl` |
...
|| `surface_spammm.cl` | SPAMMM surface: folded atomic forcefield (FAF), Ewald2D, brute Morse, isosurfaces, tensor-exp/poly FAF. **Requires** `common.cl`+`Forces.cl` first. | SPAMMM `kernels/surface.cl` |
|| `grids.cl` | Grid utilities (lattice helpers, index math). | SPAMMM `kernels/grids.cl` |
...
|| `gridff_eval.cl` | **Macro library** for GridFF sampling (eval macros injected into `getNonBonded` of UFF/SPFF/RAFF/RigidMolFF). | SurfMol |
|| `faf_build.cl` | **Macro/fragment library** for FAF construction (surface/isosurface/Ewald build kernels). | SurfMol |
|| `faf_eval.cl` | **Macro library** for FAF evaluation (folded-atomic eval macros injected into `getNonBonded`). | SurfMol |
...
- `gridff_spammm.cl` / `surface_spammm.cl` are the canonical whole-file SPAMMM sources. The `//>>>`-marked build/eval blocks have been extracted from them into the four SurfMol fragment files; the whole files are kept as reference.
