...
impl LineRenderer {
    pub fn new(device: Arc<wgpu::Device>, queue: Arc<wgpu::Queue>, color_format: wgpu::TextureFormat) -> Self {
        ...
        let camera_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("line_camera"),
            size: std::mem::size_of::<CameraData>() as u64,
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });

        Self { device, queue, pipeline, camera_buf, bind_group_layout }
    }

    pub fn render(&self, encoder: &mut wgpu::CommandEncoder, target_view: &wgpu::TextureView, depth_view: &wgpu::TextureView, camera: &CameraData, lines: &[LineVertex]) {
