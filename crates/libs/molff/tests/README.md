...
- **`test_multigrid.rs`** (~200 LOC) — 7 tests: matvec parity (TrussOp vs dense), diagonal-block parity, direct-solve parity (MG vs Gaussian), 4.7× convergence on 8×8 grid, cached coarse force parity, fitted modal quadratic parity, bend/twist orthonormality. All passing
